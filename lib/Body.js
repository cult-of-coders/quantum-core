...
Quantum.Model.Body = class {
    ...
    buildPluginAtoms(context) {
        _.each(this.registeredPlugins, (plugin, pluginName) => {
            if (plugin.requires()) {
                ...
                _.each(requirements, (requiredPluginName) => {
                    if (!this.registeredPlugins[requiredPluginName]) {
                        throw `The plugin: ${pluginName} requires: "${requirements.join(', ')}". "${requiredPluginName}" was not found.`;
                    }
                });
            }
